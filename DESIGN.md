...
- **Capítulo 1 — lo que ya jugaron:** con al menos 3 cruces registrados, la tabla y el gráfico muestran esos enfrentamientos directos (las barras de goles de siempre, `Comparison.astro`, más la nota «Cruces entre ambos: N registrados; aquí los últimos M»); sin historial suficiente, cae al fallback de los últimos 5 partidos reales por equipo, anteriores al encuentro. Fuente prioritaria: `match.lastMatches` horneado por el pipeline (Bzzoiro, últimos 5 terminados por equipo antes del kickoff); si falta, `teamForm` sobre `results.json`; si nada, ausencia honesta. Se pinta desde 1 partido por lado; el lado sin serie registrada muestra el blueprint en construcción con sus migajas (competición·fecha, tabla rescatada por `teamId`, sede si el mapa se omite, goleador si no tiene tarjeta en notas —`src/lib/series-crumbs.js`—; lo que falta se omite en silencio) y la expectativa «Sus últimos 5 aparecerán aquí…». El gráfico del fallback (`FormStrip.astro`, ver `form-strip`) es SVG dinámico generado desde los datos: una fila por equipo con un hueco por partido (marcador + rival abreviado, tintas G/E/P), parrilla fija de 5 con el más reciente a la derecha y huecos vacíos punteados; `role="img"` con la serie enumerada. Las señales por equipo viven en la columna de notas bajo el lead «De encuentros previos en su campeonato»: racha, arco en cero y mayor diferencia se calculan solo con partidos de la liga doméstica del equipo (primera competición de tablas que lo contiene y no es copa; sin liga resuelta caen en ausencia honesta), y la tarjeta de tabla y goles de temporada usa esa misma liga cuando se resuelve.
- **Capítulo 2 — waffle del resultado (`WaffleResult.astro`):** 100 cuadritos (10×10, radio 4px) que reparten los partidos imaginarios con los tonos 1X2; florecen en escalera al activarse (`.pt-fade` con `--d` de 6ms por celda) y la leyenda cuenta «N de 100». La banda λ de apertura («Lo que espera el cálculo»: λ local/visita en coma latina + partidos analizados) usa medidores con llenado `.pt-grow-x`; la doble oportunidad sigue en filas y el proveedor acompaña como **lectura lateral** en texto («La otra lectura (Bzzoiro): …»), nunca como bloque separado.
- **Capítulo 3 — escalera y podio (`GoalsLadder.astro`, `ScorePodium.astro`):** la escalera del total apila 1.5/2.5/3.5 con arranque en escalón, barra del «sí» en lima con `.pt-grow-x` escalonado y el «no» como texto («que sea probable pasar de 1.5 no dice nada de llegar a 3.5»); el podio ordena los 3 marcadores exactos en columnas lima/`#6f8173`/`#55645a` que crecen (`.grow`) con cifra Barlow 21px y resto en línea discreta. Arco en cero y «¿marcan los dos?» conservan fichas y `ProbabilityBar`; el xG y los ecos del proveedor van en la lectura lateral.
- **Capítulo 4 — primer gol y nombres:** la carrera de seis tramos conserva su geometría y sus barras ahora crecen (`.pt-grow` escalonado por tramo); «se queda en cero» sustituye al «sin goles». Las columnas de goleadores («marca en cualquier momento» + «abre el marcador» + peso) se mudan aquí desde el antiguo capítulo único.
- **Capítulo 5 — números pronosticados (`RadarDuel.astro`, cuarta pasada):** el capítulo deja de ser el acta en vivo y pasa a pronosticar las 9 cifras (posesión, tiros, a puerta, xG, córners, faltas, amarillas, rojas, fueras de juego) con el modelo de ritmos `stats-ratings-v1` (`src/lib/stats-forecast.js`: promedio propio + lo que concede el rival, encogido a la media de la competición, mínimo 3 partidos por lado y métrica; lo nulo se pinta `—`). La tabla de esperados vive en la columna de notas; el escenario abre con el **radar de dominancia** — SVG dinámico con un polígono por equipo (lima local, pizarra visita) sobre ejes de reparto con evaluación publicada, retícula de 1px, etiquetas Manrope 11px con valor en tspan, leyenda reutilizada (`.legend`) y tabla `sr-only`; florece con `.pt-fade` escalonado — seguido de los duelos en `ProbabilityBar`: «Quién saca más córners», «Primero a 3/5/7», «Por cuántos córners» (5 bandas), «Quién ve más tarjetas» y «Quién cierra el marcador». Regla de publicación: valores esperados siempre con muestra declarada; porcentajes de duelo solo cuando su insumo supera su referencia (stats: muestra ≥ 30 y MAE mejor que la media de su competición; último gol: gate Poisson). La carrera a córners hereda la evaluación de los ritmos de córner (el minutaje de córners no existe en ningún proveedor). Sin `statsForecast` el capítulo y su tab («En números») no se dibujan. El xG del proveedor acompaña como lectura lateral. Backfill del histórico vía `scripts/backfill-history.mjs` (el `eventDay` del pipeline lee `event_date`); evaluación rodante sin fuga en `scripts/evaluate-predictions.mjs` → `evaluation.stats`.
- **Vocabulario de entrada (partido):** `.pt-grow` (scaleY desde la base), `.pt-grow-x` (scaleX desde la izquierda) y `.pt-fade`, siempre bajo `.stage-pane.is-active` con `--d` escalonado y `both`; con movimiento reducido quedan en estado final. Decimales nuevos en coma latina (`~1,2`); las cifras tabulares se mantienen.
...
- **Pausas AdSense:** cuatro slots reservados (`partidoPrevio`, `partidoStory`, `partidoCancha`, `partidoCierre`) — tras la cabecera, tras el relato, tras la cancha y antes del pie; el área sticky del escenario y la cancha quedan sin publicidad.
