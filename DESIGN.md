...
### Página de partido (extensión 2026-09, tercera pasada: el relato en 6 capítulos)

El story pasa de 4 capítulos fijos a **6 capítulos dinámicos**: el esqueleto (llegada + veredicto) siempre está; resultado, goles, primer gol y números solo aparecen con datos en el JSON. El historial cara a cara se muestra en el capítulo 1 solo con al menos 3 cruces registrados (sin ellos, el capítulo cae al fallback de últimos 5) y la auditoría del LLM (`llmReview`) vive solo en el JSON, nunca en la vista. Los titulares son preguntas de aficionado a dos líneas, sin siglas: «Así llegan. / Lo que ya jugaron.» · «Quién se lo lleva. / El resultado, en 100 partidos.» · «Cuántos goles. / El partido, en el marcador.» · «El primer gol. / Cuándo cae y quién lo mete.» · «El partido, / en números.» · «El veredicto. / Con sus pesos a la vista.»
...
- **Capítulo 1 — lo que ya jugaron:** con al menos 3 cruces registrados, la tabla y el gráfico muestran esos enfrentamientos directos (las barras de goles de siempre, `Comparison.astro`, más la nota «Cruces entre ambos: N registrados; aquí los últimos M»); sin historial suficiente, cae al fallback de los últimos 5 partidos reales por equipo, anteriores al encuentro. Fuente prioritaria: `match.lastMatches` horneado por el pipeline (Bzzoiro, últimos 5 terminados por equipo antes del kickoff); si falta, `teamForm` sobre `results.json`; si nada, ausencia honesta. Se pinta desde 1 partido por lado; el lado sin serie registrada muestra el blueprint en construcción con sus migajas (competición·fecha, tabla rescatada por `teamId`, goleador si no tiene tarjeta en notas —`src/lib/series-crumbs.js`—; lo que falta se omite en silencio) y la expectativa «Sus últimos 5 aparecerán aquí…». El gráfico del fallback (`FormStrip.astro`, ver `form-strip`) es SVG dinámico generado desde los datos: una fila por equipo con un hueco por partido (marcador + rival abreviado, tintas G/E/P), parrilla fija de 5 con el más reciente a la derecha y huecos vacíos punteados; `role="img"` con la serie enumerada. Las señales por equipo viven en la columna de notas bajo el lead «De encuentros previos en su campeonato»: racha, arco en cero y mayor diferencia se calculan solo con partidos de la liga doméstica del equipo (primera competición de tablas que lo contiene y no es copa; sin liga resuelta caen en ausencia honesta), y la tarjeta de tabla y goles de temporada usa esa misma liga cuando se resuelve.
...
- **Capítulo 4 — primer gol y nombres:** la carrera de seis tramos conserva su geometría y sus barras ahora crecen (`.pt-grow` escalonado por tramo); «se queda en cero» sustituye al «sin goles» y la carrera es el único gráfico del escenario. Las columnas de goleadores («marca en cualquier momento» + «abre el marcador» + peso) viven en la columna de notas, bajo el texto del capítulo.
- **Capítulo 5 — números pronosticados (`RadarDuel.astro`, quinta pasada):** el capítulo deja de ser el acta en vivo y pasa a pronosticar las 9 cifras (posesión, tiros, a puerta, xG, córners, faltas, amarillas, rojas, fueras de juego) con el modelo de ritmos `stats-ratings-v1` (`src/lib/stats-forecast.js`: promedio propio + lo que concede el rival, encogido a la media de la competición, mínimo 3 partidos por lado y métrica; lo nulo se pinta `—`). La tabla de esperados y los duelos en `ProbabilityBar` —«Quién saca más córners», «Primero a 3/5/7», «Por cuántos córners» (5 bandas), «Quién ve más tarjetas» y «Quién cierra el marcador»— viven en la columna de notas; el escenario abre solo con el **radar de dominancia** — SVG dinámico con un polígono por equipo (lima local, pizarra visita) sobre ejes de reparto con evaluación publicada, retícula de 1px, etiquetas Manrope 11px con valor en tspan, leyenda reutilizada (`.legend`) y tabla `sr-only`; florece con `.pt-fade` escalonado. Regla de publicación: valores esperados siempre con muestra declarada; porcentajes de duelo solo cuando su insumo supera su referencia (stats: muestra ≥ 30 y MAE mejor que la media de su competición; último gol: gate Poisson). La carrera a córners hereda la evaluación de los ritmos de córner (el minutaje de córners no existe en ningún proveedor). Sin `statsForecast` el capítulo y su tab («En números») no se dibujan. El xG del proveedor acompaña como lectura lateral. Backfill del histórico vía `scripts/backfill-history.mjs` (el `eventDay` del pipeline lee `event_date`); evaluación rodante sin fuga en `scripts/evaluate-predictions.mjs` → `evaluation.stats`.
- **Quinta pasada — un protagonista por capítulo:** el escenario ya no apila gráficos. Cada pestaña conserva **un solo gráfico protagonista** (forma/H2H, radar, waffle, escalera, carrera de tramos o barras del veredicto) y todo lo demás baja a su capítulo en un bloque `.notes-visuals` (`display:flex; gap:24px;` con regla de 1px arriba), colocado tras el texto y la tabla, antes de «¿qué significa esto?» y las notas de cierre. El bloque entra una sola vez con `IntersectionObserver` (`.is-in`, umbral 0.15) que dispara el mismo vocabulario `.pt-*`; sin JS o con movimiento reducido queda visible y estático. El scroll interno de los panes largos desaparece: el escenario siempre cabe. Las notas de cierre y procedencia duplicadas se consolidan en el capítulo.
