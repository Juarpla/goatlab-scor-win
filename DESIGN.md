...
### Página de partido (extensión 2026-09, segunda pasada)

La página de partido se completa con cinco piezas del mismo mundo. Los colores de serie nuevos son tonos de datos deliberados y quedan documentados aquí: **empate** `#6f8173` y **resto/mute** `#55645a` (tercera y cuarta serie de las barras de mercados, entre la lima y la pizarra), y **amarilla** `#e9c46a` para la tarjeta en la cancha (la roja usa el tinte `#e9b4a3` del sistema). Los radios de micro-barras (2–3px) y fichas (6px) pertenecen a la escala de controles.
...
- **Capítulo de probabilidades:** cuarto capítulo del story («El guion probable, sin cuotas.») con insumos declarados (forma, λ, goleadores). El gráfico hero es la **carrera del primer gol**: seis columnas de 15 minutos, cada barra partida lima/pizarra según el reparto del primer gol, con porcentaje arriba y tramo abajo; sin goles acompaña como texto. Debajo, `ProbabilityBar.astro` apila mercados excluyentes (1X2 con empate `#6f8173`, O/U, ambos anotan) y la **doble oportunidad va en filas** (variant `rows`: una barra por opción, porque sus valores se solapan). Fichas con ícono para marcador exacto top-3, porterías a cero, más/menos 1.5/3.5, y columna de goleadores por equipo con share de temporada y mini barra. Todo se omite honestamente sin datos; el JSON fuente (`public/data/match-probabilities/{id}.json`) es supervisable a mano e incluye la auditoría del LLM como `llmReview`.
...
- **Tarjetas por equipo (`team-context`):** reemplazan al manifiesto. Dos paneles con hasta cinco datos reales cada uno (mejor jugador, goles de temporada, porterías a cero, racha sin perder, mayor diferencia), cada uno con ícono del set y su muestra declarada; lo ausente no se dibuja.
